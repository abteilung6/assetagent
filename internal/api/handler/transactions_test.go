...
func TestGetTransactions_returnsPaginatedList(t *testing.T) {
	...
	gen.HandlerWithOptions(handler.New(list, nil, nil, nil, nil, nil, nil), gen.ChiServerOptions{
		BaseRouter:       router,
...
func TestGetTransactions_invalidDate_returns400(t *testing.T) {
	router := chi.NewRouter()
	gen.HandlerWithOptions(handler.New(&stubListService{}, nil, nil, nil, nil, nil, nil), gen.ChiServerOptions{
		BaseRouter:       router,
		ErrorHandlerFunc: handler.APIErrorHandler,
	})
